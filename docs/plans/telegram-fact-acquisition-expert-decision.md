...
| Tests | New acceptance scenarios and affected regression tests pass, followed on the final candidate by `just check`, full `just unit`, `just crap-threshold`, and `just docker-build-check`, without weaker gates or unjustified exclusions. Required GitHub checks are green for the final PR head. |
