...
from mcp_telegram.tools.reading import _list_messages_structured_messages, _search_result_structured_rows
...
def _insert_scheduled(conn: sqlite3.Connection, message_id: int, at: int, text: str, state: str = "scheduled") -> None:
    conn.execute(
        """
        ...
        VALUES (1, ?, ?, ?, 99, 'Me', ?, 'author_only', 1, 1, 1700000000, 1700000000)
        """,
        (message_id, at, text, state),
    )
    conn.commit()
    conn.execute(
        "INSERT INTO scheduled_messages_fts(dialog_id, message_id, stemmed_text) VALUES (1, ?, ?)",
        (message_id, text),
    )
    conn.commit()
