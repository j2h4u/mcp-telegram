...
| `get_inbox` | Inbox | primary | true | true | yes | bootstrap_pending warning; per-chat Russian header with unread count, mentions, bot/channel labels, dialog id; read_state headers; inline_markers; media/reactions/replies/edit_date rendered through message formatter; hidden remainder marker/truncation `[и ещё N]`; personal limit/group-size/time semantics | `dialogs[] {dialog_id,name,category,unread_count,unread_mentions_count,total_in_chat,is_channel,is_bot,read_state,messages[]}`, `messages[] {msg_id,sender,date,text,media,reactions,replies,edit_date,inline_markers,untrusted_content}`, `bootstrap_pending`, `coverage.complete`, `limits {limit,group_size_threshold}`, `truncation.hidden_count_by_dialog[]` | none |
| `get_unread_summary` | Unread Summary | primary | true | true | yes | compact dialog-only unread overview; deterministic bounded top-N; persisted sweep observation metadata; requested timezone presentation | `dialogs[] {dialog_id,name,dialog_type,unread_count,unread_mark,unread_mentions_count,unread_reactions_count,archived,last_message_at}`, `count`, `total_matching`, `truncated`, `source_observation {status,completed_at,observed_count,visible_count}`, shared `time_context` | none |
...
| `list_conversation_changes` | Conversation Changes | primary | true | true | yes | one newest-first history of incoming human-DM edits/deletions and access lifecycle changes; signed snapshot pagination; kind/dialog/time filters | `events[] {event_id,kind,occurred_at,time_basis,dialog_id,dialog_title,message_id,version,summary,reason_code,access_change_cause,actor_id,text_evidence}`, `count`, `has_more`, `next_navigation`, `coverage` | none |
...
| `access_lost` | `list_dialogs` access_lost_at; `list_conversation_changes` durable history; archived warning | `dialogs[].access_lost_at`, `events[].kind="access_lost"`, `coverage.access_changes` |
