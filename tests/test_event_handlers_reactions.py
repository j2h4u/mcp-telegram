...
def _insert_msg(conn: _SQLiteConnection, dialog_id: int, message_id: int, text: str = "hi") -> None:
    conn.execute(
...
def _insert_reaction(conn: _SQLiteConnection, dialog_id: int, message_id: int, emoji: str, count: int) -> None:
    conn.execute(
...
def _reactions(conn: _SQLiteConnection, dialog_id: int, message_id: int) -> list[tuple[str, int]]:
    return [
        (str(row[0]), int(row[1]))
...
async def test_on_message_edited_reactions_only_applies_delta(
    ...
    event = cast(_EditedMessageEvent, SimpleNamespace(chat_id=dialog_id, message=msg))
...
@pytest.mark.asyncio
async def test_on_raw_reaction_update_user_peer(
    mock_client: MagicMock,
    sync_db: _SQLiteConnection,
    shutdown_event: asyncio.Event,
) -> None:
...
@pytest.mark.asyncio
async def test_on_raw_reaction_update_channel_peer(
    mock_client: MagicMock,
    sync_db: _SQLiteConnection,
    shutdown_event: asyncio.Event,
) -> None:
...
@pytest.mark.asyncio
async def test_on_raw_reaction_removal(
    mock_client: MagicMock,
    sync_db: _SQLiteConnection,
    shutdown_event: asyncio.Event,
) -> None:
...
async def test_on_raw_reaction_update_drops_silently_for_unsynced_dialog(
    ...
    dialog_id = 999999
    # Do NOT enroll
    update = cast(_RawReactionUpdate, SimpleNamespace(peer=PeerUser(user_id=dialog_id), msg_id=10))
...
async def test_idempotency_edited_then_raw_same_state(
    ...
    edited_event = cast(_EditedMessageEvent, SimpleNamespace(chat_id=dialog_id, message=edited_msg))
