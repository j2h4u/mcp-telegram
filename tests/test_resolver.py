...
def test_resolve_exact_match(sample_entities: dict) -> None:
    from mcp_telegram.resolver import resolve, Resolved
...
def test_numeric_query(sample_entities: dict) -> None:
    from mcp_telegram.resolver import resolve, Resolved, NotFound
...
def test_ambiguity(sample_entities: dict) -> None:
    from mcp_telegram.resolver import resolve, Candidates

    # Both "Ivan Petrov" and "Ivan's Team" start with "Ivan" — both should score >=90 with WRatio
...
def test_not_found(sample_entities: dict) -> None:
    from mcp_telegram.resolver import resolve, NotFound
