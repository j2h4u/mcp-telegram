...
### Phase 999.6: ~~Резолвинг имён для пересылок~~ — ИССЛЕДОВАНО, НЕ АКТУАЛЬНО (BACKLOG)

**Goal:** ~~Пересланные сообщения с `peer_id` без `from_name` корректно отображают имя отправителя~~

**Requirements:** N/A — не требует реализации
**Plans:** 0 plans

**Вывод (2026-04-24):**

Гипотеза опровергнута. После полного ресинка диалога (Ольга Голева, 1468 сообщений, 15 батчей) в логах не появилось **ни одного** вызова `_resolve_peer_name`. Это означает: все 31 пересылок с пустым `fwd_from_name` имеют `from_id = None` — анонимные форварды, где Telegram намеренно скрывает источник. Резолвинг принципиально невозможен: идентификатора нет. Проблемы в коде нет.

Попутно улучшена обработка ошибок в `_resolve_peer_name`: заменён голый `except Exception` на типизированные ветки с логированием (commit 54d9e45).
