...
class DaemonAPIServer:
    ...
    async def _dispatch(self, req: dict) -> dict:
        ...
        if method == "describe_source":
            return await self._describe_source(req)
        if method == "export_source_changes":
            return await self._export_source_changes(req)
    ...
    def _source_row_to_change(self, row: sqlite3.Row) -> dict:
        dialog_id = int(row["dialog_id"])
        message_id = int(row["message_id"])
        document_ref = f"dialog:{dialog_id}"
        unit_ref = self._unit_ref(dialog_id, message_id)
        unit_updated_at = self._source_iso(int(row["unit_updated_epoch"]))
        ...
        unit_metadata = {
            ...
            "sent_at": self._source_iso(int(row["sent_at"])),
            "sender_id": row["sender_id"],
            "sender_name": row["sender_first_name"],
            "topic_id": row["forum_topic_id"],
            "topic_title": row["topic_title"],
            "reply_to_msg_id": row["reply_to_msg_id"],
            "edit_date": self._source_iso(int(edit_date)) if edit_date is not None else None,
            "deleted_at": self._source_iso(int(row["deleted_at"])) if row["deleted_at"] is not None else None,
    ...
    async def _describe_source(self, req: dict) -> dict:
        return {
            "ok": True,
            "data": {
                "namespace": "telegram",
    ...
    async def _export_source_changes(self, req: dict) -> dict:
        try:
            cursor_key = self._parse_source_cursor(req.get("cursor"))
            updated_after_cursor = self._parse_source_cursor(req.get("updated_after_cursor"))
            updated_after_epoch = self._parse_source_watermark(req.get("updated_after"))
        except ValueError as exc:
        ...
        if updated_after_epoch is not None and remaining > 0:
            update_rows = self._source_rows_after_update_watermark(
                updated_after_epoch,
        ...
        if update_rows:
            ...
            updated_after_cursor_out = self._source_cursor(
    ...
    async def _read_source_unit_window(self, req: dict) -> dict:
        try:
            dialog_id, message_id = self._parse_unit_ref(req.get("unit_ref"))
        except ValueError as exc:
            return {"ok": False, "error": str(exc)}
        ...
        return {
            ...
            "data": {
                ...
                "document_ref": f"dialog:{dialog_id}",
                "unit_ref": self._unit_ref(dialog_id, message_id),
                "units": [self._source_row_to_change(row)["unit"] for row in rows],
