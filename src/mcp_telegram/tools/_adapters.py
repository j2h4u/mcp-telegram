...
class DaemonMessage:
    ...
    __slots__ = (  # noqa: RUF023  order mirrors format_messages access pattern, not alphabetical
        "id",
        "date",
        "message",
        "sender",
    ...
    def __init__(self, row: dict) -> None:
        ...
        self.date = datetime.fromtimestamp(int(sent_at), tz=UTC)
        ...
        if edit_date_raw is not None:
            self.edit_date: datetime | None = datetime.fromtimestamp(int(edit_date_raw), tz=UTC)
        else:
            self.edit_date = None
