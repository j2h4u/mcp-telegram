...
from .account_trace_sqlite import (
    ...
    hidden_dialog_ids,
    retry_fragment_dialog_ids,
    visible_synced_dialog_ids,
)
...
def _resolve_trace_account_by_id(
    ...
    *,
    ...
    row = account_by_id(conn, account_id)
    if row is None:
...
def _resolve_trace_account_by_username(conn: sqlite3.Connection, username: str) -> dict | None:
    row = account_by_username(conn, username)
    if row is None:
        return None
...
def _resolve_trace_account_by_fuzzy(
    ...
    *,
    ...
    names, normalized_rows = account_directory_names(
        conn,
        user_after=now - user_directory_ttl_seconds,
        group_after=now - group_directory_ttl_seconds,
    )
    for values in names:
    ...
    if isinstance(result, Resolved):
        row = account_by_id(conn, result.entity_id)
        if row is not None:
...
async def _resolve_trace_account_signature_scope(
    ...
    meta = dialog_metadata(deps.conn, exact_dialog_id)
    if (
...
def _collect_trace_gaps_for_hidden_dialogs(
    ...
    *,
    ...
    hidden_dialogs = hidden_dialog_ids(request.conn)
    return [
        _trace_gap(
            _TraceGapCandidate(
                kind="hidden_dialog",
...
def _add_trace_candidate_fragments(
    ...
    *,
    ...
    request = state.request
    for dialog_id in retry_fragment_dialog_ids(request.conn, target_user_id=request.target_user_id, now=now):
        _add_trace_candidate_dialog(
            state=state,
            dialog_id=dialog_id,
...
def _add_trace_candidate_common_chats(
    ...
    request = state.request
    for dialog_id in common_chat_ids(request.conn, request.target_user_id):
        _add_trace_candidate_dialog(
            state=state,
            dialog_id=dialog_id,
...
def _add_trace_candidate_visible_synced(
    ...
    request = state.request
    for dialog_id in visible_synced_dialog_ids(request.conn):
        _add_trace_candidate_dialog(
            state=state,
            dialog_id=dialog_id,
