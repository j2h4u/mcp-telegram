...
Topic attribution receipts are published by the normal full-history owner.
Legal root or General-topic `NULL` outcomes remain valid and are reported as
`topic_attribution.no_topic_count`; they do not change an empty topic selection
from `unknown` to absence.
